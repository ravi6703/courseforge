        ...
        <footer className="flex items-center justify-between mt-6 pt-4 border-t border-bi-navy-100">
          ...
          {step < 4 ? (
            ...
            <button
              onClick={finish}
              disabled={saving}
              className="px-5 py-2 rounded-md bg-bi-navy-900 text-white text-[13px] font-semibold hover:bg-bi-navy-800 disabled:opacity-50 inline-flex items-center gap-1.5"
            >
      ...
      {[1, 2, 3, 4].map((n) => (
        <div key={n} className="flex items-center flex-1">
          <span className={`w-7 h-7 rounded-full grid place-items-center text-[11px] font-bold transition-colors ${
            ...
            n === step ? "bg-bi-navy-900 text-white" :
