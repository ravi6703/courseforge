...
export async function CourseHeader({ courseId }: { courseId: string }) {
  ...
  if (!course) {
    return (
      <div className="bg-white border-b border-slate-200 px-7 py-5">
        <Link href="/dashboard" className="text-[13px] text-slate-600 hover:text-slate-900 flex items-center gap-1.5 w-fit">
  ...
  const createdRel = relativeTime(course.created_at);

  return (
    <div className="bg-white border-b border-slate-200 px-7 pt-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="min-w-0">
          <div className="text-[10.5px] uppercase tracking-[.06em] font-bold text-slate-500">
            {course.platform || "internal"}
            {course.domain && ` · ${course.domain}`}
            {course.duration_weeks && ` · ${course.duration_weeks} weeks`}
          </div>
          <h1 className="text-[22px] font-extrabold text-slate-900 tracking-tight mt-1">{course.title}</h1>
          <div className="mt-1.5 flex items-center gap-3 text-[13px] text-slate-500 flex-wrap">
            <span className="inline-flex items-center gap-1.5"><AvatarMini name="Ravi Bohra" /> Ravi Bohra <span className="text-slate-400">· coach</span></span>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <Link href={`/dashboard?settings=${courseId}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 text-[13px] font-semibold text-slate-700 hover:bg-slate-50">
