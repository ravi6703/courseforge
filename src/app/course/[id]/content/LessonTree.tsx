...
export function LessonTree({ rows, selectedVideoId, onSelect }: LessonTreeProps) {
  ...
  return (
    <aside className="bg-white border border-bi-navy-100 rounded-[10px] shadow-bi-sm flex flex-col overflow-hidden">
      {/* Header */}
      <div className="px-4 py-3 border-b border-bi-navy-100">
        <div className="text-[10px] font-bold uppercase tracking-[.06em] text-bi-navy-500">Course progress</div>
        <h3 className="mt-0.5 text-[14px] font-bold text-bi-navy-900 tracking-tight">
          {approved} <span className="text-bi-navy-300 font-semibold">/ {totalArt}</span>
          <span className="text-bi-navy-500 font-medium ml-1.5">artifacts · {pct}%</span>
        </h3>
        <div className="mt-2 h-1.5 rounded-full bg-bi-navy-100 overflow-hidden">
      ...
      <div className="px-3 py-2.5 border-b border-bi-navy-100">
        <div className="flex items-center gap-2 border border-bi-navy-100 rounded-md px-2.5 py-1.5 focus-within:border-bi-blue-600 focus-within:ring-2 focus-within:ring-bi-blue-100 transition-all">
      ...
      <div className="flex-1 overflow-auto py-2">
          ...
          return (
            <div key={modTitle} className="mt-1.5 first:mt-0">
              <div className="px-4 pt-1.5 pb-1 flex items-center justify-between">
                <span className="text-[10.5px] font-bold uppercase tracking-[.05em] text-bi-navy-500">{modTitle}</span>
                <span className="text-[10px] font-semibold text-bi-navy-400">{modPct}%</span>
              </div>
              {Object.entries(lessons).map(([lessonTitle, videos]) => (
                <div key={lessonTitle} className="px-4 pt-1 pb-0.5">
                  <div className="text-[12px] font-semibold text-bi-navy-700 truncate">{lessonTitle}</div>
                </div>
              ))}
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left flex items-center gap-2 px-4 py-1.5 text-[13px] border-l-[3px] transition-colors ${
                      isSel
                        ? "bg-bi-blue-50 border-l-bi-blue-600 text-bi-navy-900 font-bold"
                        : "border-l-transparent text-bi-navy-700 hover:bg-bi-navy-50"
                    }`}
                  >
                    <span className="font-mono text-[10px] font-bold text-bi-navy-400 shrink-0 tabular-nums w-6">
        ...
        {rows.length === 0 && (
          <div className="px-4 py-8 text-center text-[13px] text-bi-navy-500">
