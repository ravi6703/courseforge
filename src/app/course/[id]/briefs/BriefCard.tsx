...
export function BriefCard({
  ...
  return (
    <div className={embedded
      ? "bg-transparent"
      : `rounded-lg border bg-white overflow-hidden ${isApproved ? "border-emerald-300" : "border-bi-navy-200"}`}>
      {/* Header — hidden in embedded mode (BriefsView shows it) */}
      {!embedded && (
      <div className="px-4 py-3 flex items-center justify-between gap-4">
        <div className="min-w-0 flex-1">
          <div className="text-xs text-bi-navy-500 truncate">
            {moduleTitle} <span className="text-bi-navy-300 mx-1">›</span>
            {lessonTitle} <span className="text-bi-navy-300 mx-1">›</span>
            <span className="text-bi-navy-600">{videoTitle}</span>
          </div>
          <div className="font-medium text-bi-navy-900 truncate">{videoTitle}</div>
        </div>
          ...
          {brief ? (
            <>
              ...
              <button
                ...
                className="text-bi-navy-400 hover:text-purple-600 px-2 py-0.5 rounded border border-bi-navy-200 hover:border-purple-300 text-xs inline-flex items-center gap-1"
              >
                <Wand2 className="w-3.5 h-3.5" /> Suggest
              </button>
              <button
                onClick={() => setView(view === "input" ? "brief" : "input")}
                className="text-bi-navy-400 hover:text-bi-navy-600 text-xs px-2 py-0.5 rounded border border-bi-navy-200 hover:border-bi-navy-300"
              ...
              <button onClick={generate} disabled={loading} title="Regenerate" className="text-bi-navy-400 hover:text-bi-navy-600 disabled:opacity-40">
                <RefreshCw className={`w-4 h-4 ${loading ? "animate-spin" : ""}`} />
              </button>
              <button onClick={() => setExpanded(!expanded)} className="text-bi-navy-400 hover:text-bi-navy-600">
            ...
            <button
              onClick={() => setView(view === "input" ? "brief" : "input")}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-bi-navy-100 text-bi-navy-700 text-xs hover:bg-bi-navy-200 transition-colors"
            >
      ...
      {embedded && (
        <div className="flex items-center justify-end gap-2 px-2 pb-2">
          ...
          {brief && (
            <>
              <button onClick={toggleApproval} disabled={approving} className={`text-xs px-2 py-1 rounded border inline-flex items-center gap-1 disabled:opacity-40 ${isApproved ? "border-bi-navy-300 text-bi-navy-700 hover:bg-bi-navy-50" : "border-emerald-400 text-emerald-700 bg-emerald-50 hover:bg-emerald-100"}`} title={isApproved ? "Revoke approval" : "Approve for slide generation"}>
                {isApproved ? <RotateCcw className="w-3 h-3" /> : <Check className="w-3 h-3" />}
                {approving ? "…" : (isApproved ? "Unapprove" : "Approve")}
              </button>
              <button onClick={() => setSuggestOpen((v) => !v)} title="Suggest improvements" className="text-bi-navy-400 hover:text-purple-600 px-2 py-0.5 rounded border border-bi-navy-200 hover:border-purple-300 text-xs inline-flex items-center gap-1">
                <Wand2 className="w-3.5 h-3.5" /> Suggest
              </button>
              <button onClick={() => setView(view === "input" ? "brief" : "input")} className="text-bi-navy-400 hover:text-bi-navy-600 text-xs px-2 py-0.5 rounded border border-bi-navy-200 hover:border-bi-navy-300" title={view === "input" ? "View brief" : "Edit coach input"}>
                {view === "input" ? <FileText className="w-3.5 h-3.5" /> : <ClipboardList className="w-3.5 h-3.5" />}
              </button>
              <button onClick={generate} disabled={loading} title="Regenerate" className="text-bi-navy-400 hover:text-bi-navy-600 disabled:opacity-40">
                <RefreshCw className={`w-4 h-4 ${loading ? "animate-spin" : ""}`} />
          ...
          {!brief && (
            <button onClick={() => setView(view === "input" ? "brief" : "input")} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-bi-navy-100 text-bi-navy-700 text-xs hover:bg-bi-navy-200">
              <ClipboardList className="w-3.5 h-3.5" />
              {view === "input" ? "Hide form" : "Coach input"}
            </button>
          )}
      ...
      {/* Coach Input Form */}
      {view === "input" && (
        <div className="border-t border-bi-navy-100 px-4 py-4 bg-bi-navy-50/50 space-y-3">
      ...
      {brief && view === "brief" && expanded && (
        <div className="border-t border-bi-navy-100 px-4 py-4 space-y-4 text-sm">
          ...
          {brief.script_outline && (
            <div>
              <div className="text-xs font-semibold text-bi-navy-500 uppercase tracking-wider mb-2">Script Outline</div>
              <pre className="text-xs font-mono text-bi-navy-700 whitespace-pre-wrap bg-bi-navy-50 rounded-md p-3 border border-bi-navy-200">
                {brief.script_outline}
              </pre>
            </div>
          )}
              ...
              {suggestion && (
                <div className="space-y-3">
                  ...
                  {suggestion.script_outline && (
                    <pre className="text-xs font-mono text-bi-navy-700 whitespace-pre-wrap bg-white rounded-md p-2 border border-purple-200">{suggestion.script_outline}</pre>
                  )}
                  <div className="flex gap-2">
          ...
          <div className="rounded-md border border-dashed border-bi-navy-300 p-3 flex items-center justify-between">
            <p className="text-xs text-bi-navy-500">
              {isApproved
...
function InputField({ label, placeholder, value, onChange, rows = 2 }: { label: string; placeholder: string; value: string; onChange: (v: string) => void; rows?: number }) {
  return (
    <div>
      <label className="block text-xs font-medium text-bi-navy-600 mb-1">{label}</label>
      <textarea
        className="w-full text-xs border border-bi-navy-200 rounded-md px-2.5 py-1.5 bg-white resize-none focus:outline-none focus:ring-1 focus:ring-bi-blue-400 focus:border-blue-400 placeholder:text-bi-navy-300"
  ...
  return (
    <div>
      ...
      <input
        ...
        min={1}
        max={180}
        className="w-full text-xs border border-bi-navy-200 rounded-md px-2.5 py-1.5 bg-white focus:outline-none focus:ring-1 focus:ring-bi-blue-400 focus:border-blue-400 placeholder:text-bi-navy-300"
        placeholder={placeholder}
        value={value}
        onChange={(e) => onChange(e.target.value)}
      />
      {hint && <p className="text-[10px] text-bi-navy-400 mt-0.5">{hint}</p>}
    </div>
  );
}
...
function BriefSection({ title, items }: { title: string; items: string[] }) {
  ...
  return (
    <div>
      <div className="text-xs font-semibold text-bi-navy-500 uppercase tracking-wider mb-1.5">{title}</div>
      <ul className="space-y-1">
        {items.map((item, i) => (
          <li key={i} className="flex gap-2 text-bi-navy-700">
